...
def fixing_is_st(start, end):
    """
    st历史信息是从choice金融终端得到的,choice批量数据获取方式是excel,因单次获取数据大小有限制,
    故按照时间分成了4个年份;
    
    根据另一张记录了st摘帽和戴帽日期的表,获取到哪些表曾经st过,故不需要对所有的股票都进行is_st的填充;
    """
...
def fill_high_and_low_price_between(start, end):
    ...
    2.每只股票填充范围限定在[timeToMarket, end], 查找数据库
    
    3.对2中返回的cursor进行循环
        3.1. 首先获取pre_close; 如果没有获取到, 分3种情况:
            a. 数据初始日, 且不是股票的上市日
            b. 股票的上市日, 涨跌停计算
            c. 其他情况,就是有错误了
        3.2. 判断股票是否为st的情况, 如果是, 计算涨跌停
        3.3. 通常情况下涨跌停的计算
